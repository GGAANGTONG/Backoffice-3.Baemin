...
initializeDatabase().then(() => {
    ...
    router.post('/test', async (req, res, next) => {
        ...
        await entityManager.transaction(async transactionalEntityManager => {
            const user = await transactionalEntityManager.findOne('users', { where: { userId: userId } });
            const menu = await transactionalEntityManager.findOne('menu', { where: { menuId: menuId } });
            // const menu = {
            //     "menuId": 1,
            //     "storeId": 1,
            //     "name": "게살 피자",
            ...
            if (user.point < menu.price) {
                return res.status(400).json({ message: '포인트 부족으로인해 주문이 불가능 합니다.' });
            }
